...
class SvenwebSummaryViewlet(BlankSlateViewlet):
    blank_template = ZopeTwoPageTemplateFile('blank_slate.pt')
...
class SvenwebFeedAdapter(BaseFeedAdapter):
    implements(IFeedData)
    adapts(IProject)

    title = "Wikis"
    itemstitle = "recent edits"

    @property
    def link(self):
        return "%s/wikis/" % self.context.absolute_url()
    ...
    def populate_items(self, n_items=5):
        self._items = []
        base_uri = getUtility(IProvideSiteConfig).get('svenweb uri')
        uri = '%s/feed/' % base_uri
